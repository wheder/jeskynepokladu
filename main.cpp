...
double spocitej(int filled, int * temp_arr, int new_index , double V_so_far, double C_so_far) {
    ...
    cout << "index_added " << new_index << endl;
    cout << "     Vsofar " << V_so_far << endl;
    cout << "     Csofar " << C_so_far << endl;
#endif /* __TEST */
    if (V_so_far > max_objem) return -1.0;

    if (C_so_far > max_sum) {
#ifdef __TEST
        cout << "____________OVERWRITE " << C_so_far  <<endl;
#endif /* __TEST */
        ret = C_so_far;
        max_sum = C_so_far;
        max_cnt = filled+1;
    ...
    while (index < pocet) {
        ...
        //memcpy(&temp_array_test, &temp_arr, pocet*sizeof(int));

        for (int g =0; g<pocet; g++) temp_array_test[g]=temp_arr[g];

#ifdef __TEST
        cout << "index_called " << index << endl;
#endif /* __TEST */
        double max = spocitej(filled+1, temp_array_test, index, V_so_far, C_so_far);
#ifdef __TEST
        cout << "max " <<max <<endl;
        cout << "filled " <<filled+1 <<endl;
#endif /* __TEST */
        if (max > max_sum) {
#ifdef __TEST
            cout << "____________OVERWRITE " << max    <<endl;
#endif /* __TEST */
            ret = max;
            max_sum = max;
            max_cnt = filled+2;
...
#ifdef __TEST
        cout << "_____________NEW F "<<f<<endl;
#endif /* __TEST */
