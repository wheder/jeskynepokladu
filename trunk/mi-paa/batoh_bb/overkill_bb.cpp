...
double stavy = 0;
void spocitej(bool * temp_arr, int new_index , int M_so_far, int C_so_far, int rest_sum) {
        stavy++;

	M_so_far += M[new_index];
	C_so_far += C[new_index];
        rest_sum -= C[new_index];

        if (M_so_far > max_hmotnost) return;
        if (C_so_far + rest_sum <= max_sum) return;
	...
	while (index < pocet) {
                spocitej(temp_arr, index, M_so_far, C_so_far, rest_sum);
                rest_sum -=C[index];
                index++;
	}
