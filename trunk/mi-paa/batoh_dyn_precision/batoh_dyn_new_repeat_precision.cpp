...
int main(int argc, char *argv[])
{
	...
	if (argc != 4) {
		cout << "usage: " <<argv[0]<< " <inputfile> <instances> <precision>" << endl;
		return 1;
	}
	...
	if (precision < 1) {
		cerr << "Precision has to be greater than 0." << endl;
		return 8;
	}
	...
	for (int inst=0; inst < instances_count; ++inst) {
		...
		max_hmotnost = floor(max_hmotnost/(float)precision);
		C = new int[pocet]; // cost
		M = new int[pocet]; // objem
		for ( int i=0 ; i<pocet ; i++ ) {
			inf>>M[i];
			if (!inf.good()) return 4;
			M[i] = floor(M[i]/(float)precision);
			inf>>C[i];
		...
		printf("%d %d %d",id, dosazena_cena, precision);
